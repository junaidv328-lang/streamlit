...
# ══════════════════════════════════════════════
# CACHED SMARTAPI FETCHER (rate-limit hardened)
# ══════════════════════════════════════════════
...
def fetch_candles_cached(
    ...
    """
    Wraps obj.getCandleData with:
      • In-memory cache keyed by cache_key with TTL
      • Exponential backoff retry on rate-limit / timeout errors
      • Graceful fallback: returns last cached value if all retries fail

    Returns: dict with keys {"status": bool, "data": list-of-rows, "message": str,
                              "from_cache": bool, "cache_age_sec": float}
    """
    ...
    # Retry loop — 3 attempts with backoff: 0s, 2s, 5s
    last_err = None
    for attempt in range(3):
        if attempt > 0:
            _time.sleep([2, 5][attempt - 1])
        try:
            params = {
                "exchange":    exchange,
                "symboltoken": token,
            ...
            if resp.get("status"):
                # Success — cache it
                clean = {"status": True, "data": resp["data"],
                         "message": resp.get("message", "ok")}
                cache[cache_key] = (now, clean)
                return {**clean, "from_cache": False, "cache_age_sec": 0.0}

            # API returned status=false (e.g. rate-limit message)
            msg = str(resp.get("message", "")).lower()
            if "access rate" in msg or "rate" in msg or "throttle" in msg:
                last_err = f"Rate-limited (attempt {attempt + 1}/3): {resp.get('message')}"
                continue
            # Non-rate-limit failure — return immediately
            return {"status": False, "data": [], "message": resp.get("message", "fetch failed"),
                    "from_cache": False, "cache_age_sec": 0.0}

        except Exception as e:
            err_str = str(e).lower()
            last_err = f"Exception (attempt {attempt + 1}/3): {e}"
            # Only retry on transient errors
            if not any(k in err_str for k in ["timeout", "timed out", "rate", "json", "connection"]):
                break

    # All retries failed → fall back to stale cache if present
    if cache_key in cache:
        ...
        return {
            **cached_resp,
            "from_cache": True,
            "cache_age_sec": age,
            "message": f"Using stale cache ({age:.0f}s old). Last error: {last_err}",
        }

    return {"status": False, "data": [], "message": last_err or "Unknown fetch error",
            "from_cache": False, "cache_age_sec": 0.0}
...
with tab3:
    ...
    with cc4:
        refresh_interval = st.selectbox(
            "Refresh",
            ["Off", "60s", "90s", "120s", "300s"],
            index=1,  # default 60s
            key="live_refresh_interval",
            help="How often the app re-fetches data from SmartAPI. Cached results may still be reused.",
        )
    ...
    with fr_col1:
        force_refresh = st.button("🔄 FORCE REFRESH", key="live_force_refresh",
                                   help="Bypass cache and pull fresh data now")

    st.markdown("</div>", unsafe_allow_html=True)

    # Auto-refresh
    refresh_secs_map = {"Off": 0, "60s": 60, "90s": 90, "120s": 120, "300s": 300}
    refresh_secs = refresh_secs_map[refresh_interval]
    if refresh_secs > 0:
        try:
            from streamlit_autorefresh import st_autorefresh
            st_autorefresh(interval=refresh_secs * 1000, key="brooks_autorefresh")
        except ImportError:
            st.markdown(
    ...
    # ── Fetch latest candles (cached + retry) ────
    ...
    try:
        ...
        # Cache key: tied to symbol/timeframe/lookback so each combo is cached separately
        cache_key = f"{live_symbol}|{live_tf}|{lookback_days}d"
        # TTL slightly less than refresh interval to allow one fresh fetch per cycle
        ttl = max(25, refresh_secs - 5) if refresh_secs > 0 else 60

        result = fetch_candles_cached(
            obj=obj,
            exchange=exchange,
            token=token,
            interval=INTERVAL_MAP[live_tf],
            from_dt_str=from_dt.strftime("%Y-%m-%d %H:%M"),
            to_dt_str=to_dt.strftime("%Y-%m-%d %H:%M"),
            cache_key=cache_key,
            ttl_seconds=ttl,
            force=force_refresh,
        )
        ...
        if not result.get("status"):
            fetch_placeholder.error(f"Fetch failed: {result.get('message')}")
            st.stop()
        ...
        # Different badge depending on cache state
        if result.get("from_cache"):
            age = result.get("cache_age_sec", 0)
            badge_color = "badge-amber" if age < ttl * 1.5 else "badge-red"
            cache_note = f"📦 CACHED ({age:.0f}s old)"
        else:
            badge_color = "badge-green"
            cache_note = "✓ FRESH"

        fetch_placeholder.markdown(
            f'<span class="badge {badge_color}">{cache_note} · '
            f'{len(df_live)} candles · last bar: '
            f'{last_ts_display.strftime("%H:%M:%S")} · '
            f'as of {now_ts.strftime("%H:%M:%S")}</span>',
            unsafe_allow_html=True,
        )
    except Exception as e:
